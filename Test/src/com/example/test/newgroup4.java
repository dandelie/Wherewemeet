...
public class newgroup4 extends Activity {
	String ID,roomname;
	...
	private final new4roomHandler n4_handler=new new4roomHandler(this);
	  
	private static class new4roomHandler extends Handler{
		private final WeakReference<newgroup4> aActivity;
		public new4roomHandler(newgroup4 activity){
			aActivity = new WeakReference<newgroup4>(activity);
		}
		@Override
		public void handleMessage(Message msg){
			newgroup4 activity=aActivity.get();
			if(activity!=null){
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.newgroup4);
    ...
    public void new4a(View v){
        Intent intent=new Intent(getApplicationContext(), newgroup5.class);
        intent.putExtra("daycheck", daycheck);
        intent.putExtra("아이디", ID);
        intent.putExtra("방이름", roomname);
		intent.putExtra("멤버리스트", memberlist);
		intent.putExtra("placecheck",0);
		if(daycheck==1){
    ...
    public void new4b(View v){
        Intent intent=new Intent(getApplicationContext(), newgroup6.class);
        intent.putExtra("daycheck", daycheck);
        intent.putExtra("아이디", ID);
        intent.putExtra("방이름", roomname);
		intent.putExtra("멤버리스트", memberlist);
		
		intent.putExtra("placecheck",1);
    ...
    public void new4c(View v){
    	...
    	thread = new Thread(){
			public void run() {
				...
				client = s.getsocket();	
				if(daycheck==0){
					roomThread = new RoomThread(client, n4_handler, roomname, memberlist,daycheck,placecheck,6);
				}//비정기모임
				else{
					roomThread = new RoomThread(client, n4_handler, roomname, memberlist,daycheck,placecheck,day,starttime,dayreturnvalue,7);
				}//정기모임
				roomThread.start();
			}
		};
		thread.start();
    }//먼 장소에서 만나도 괜찮을 때 - 바로 방생성
